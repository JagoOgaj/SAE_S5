...
class Service_USER:
    ...
    def getUserConversationsByPeriod(self: Self, limit: int, page: int = 1) -> dict:
        ...
        conversations = MODEL_CONVERSATION.objects(user_id=self._curentUserId).order_by('-created_at').skip(skip_count).limit(limit)
    ...
    def deleteConversation(self: Self, idConversation: int) -> None:
        ...
        conversation = MODEL_CONVERSATION.objects(id=idConversation, user_id=self._curentUserId).first()
    ...
    def _add_images_to_conversation(self: Self, conversation: MODEL_CONVERSATION, images: list) -> None:
        for image in images:
