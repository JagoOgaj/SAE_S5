...
from backend.app.core.utility.utils import preprocess_images_GAS, preprocess_images_GAT
import numpy as np
...
class Service_MODEL:
    ...
    def __preprocess_image(self, imageBase64) -> list:
        if any(
            ...
            return preprocess_images_GAS(
                imageBase64,
                image_size,
                needGray=(
                    False
                    if self._typeModel == ENUM_MODELS_TYPE.GENDER_AND_AGE_SCRATCH
                    else True
                ),
            )
    ...
    def get_prediction(self: Self, imageBase64) -> str:
        ...
        match self._typeModel:
            ...
            case ENUM_MODELS_TYPE.AGE_SCRATCH.value:
                return f"{round(MinMaxScaler().inverse_transform(self._model.predict(img))[0][0])} ans"

            case ENUM_MODELS_TYPE.GENDER_AND_AGE_SCRATCH.value:
                gender_pred, age_pred = self._model.predict(img)
                return f"{'Homme' if gender_pred[0][0] < 0.5 else 'Femme'} de {age_pred[0][0]:.2f} ans"
            case ENUM_MODELS_TYPE.GENDER_AND_AGE_TRANSFER.value:
                ...
                return f"{gender_mapping[pred_gender]} de {int(pred_age)} ans"
            case _:
                ModelTypeNotFoundError(self._typeModel)

    def __get_image_size(self: Self) -> tuple[int, int]:
        if (
            self._typeModel == ENUM_MODELS_TYPE.GENDER_AND_AGE_SCRATCH.value
            or self._typeModel == ENUM_MODELS_TYPE.AGE_SCRATCH.value
        ):
            return (128, 128)
